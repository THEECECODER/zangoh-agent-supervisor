...
const AgentConfig=()=>{
 ...
 return <Box>
  <Flex justify="space-between" align={{base:'start',md:'center'}} mb={5} gap={3} flexWrap="wrap">
   <Box><Text fontSize="10px" color="textSecondary" fontWeight="800">SUPERVISOR CONTROL ROOM</Text><Heading size={{base:'md',md:'lg'}}>CSR-Returns · Control Room</Heading><Text color="textSecondary" fontSize="11px">Configure, evaluate, and deploy agent behavior with governed change controls.</Text></Box>
  ...
  <Grid templateColumns={{base:'1fr',lg:'230px 1fr',xl:'230px minmax(0,1.45fr) 300px'}} gap={3} alignItems="start">
   <Box bg="surface" border="1px solid" borderColor="border" borderRadius="10px" p={4}>
    ...
    <Tabs size="sm" variant="soft-rounded" colorScheme="purple"><TabList mb={4}><Tab fontSize="9px">Parameters</Tab><Tab fontSize="9px">Capabilities</Tab><Tab fontSize="9px">Knowledge</Tab></TabList><TabPanels>
     ...
     <TabPanel p={0}><Text fontSize="10px" color="textSecondary">Connected knowledge bases</Text><Text mt={3} fontSize="11px">Returns policy · Product catalogue · Shipping rules</Text></TabPanel>
    </TabPanels></Tabs>
    <Box mt={4} p={3} bg="surfaceSubtle" borderRadius="8px"><Text fontSize="8px" color="textSecondary">CHANGE REQUEST</Text><Text fontSize="10px" fontWeight="700">CR-1842 · 6 fields modified</Text><Badge mt={2} colorScheme="yellow">Risk: Moderate</Badge></Box>
   </Box>
   <Box bg="surface" border="1px solid" borderColor="border" borderRadius="10px" p={4}>
    <Flex justify="space-between" mb={4}><Box><Heading size="sm">Performance & guardrails</Heading><Text fontSize="9px" color="textSecondary">124 scenarios across policy, tone, safety, and task completion</Text></Box><Badge>Last 7 days</Badge></Flex>
    <Grid templateColumns="1fr 1fr" gap={3} mb={5}>{[['CONTAINMENT','74.6%','+3.2%'],['CSAT','8.8','+0.4'],['FALLBACK','11.2%','-2.1%'],['HALLUCINATION','0.7%','-0.3%']].map(([a,b,c])=><Box key={a} p={3} bg="surfaceSubtle" borderRadius="8px"><Text fontSize="8px" color="textSecondary">{a}</Text><Flex justify="space-between" align="end"><Text fontWeight="800" fontSize="17px">{b}</Text><Text fontSize="8px" color="green.500">{c}</Text></Flex></Box>)}</Grid>
    ...
    <Divider my={5}/><Text fontSize="11px" fontWeight="800" mb={2}>Routing matrix</Text><Text fontSize="8px" color="textSecondary" mb={2}>Traffic split by intent and customer tier</Text>
   ...
   <Box bg="surface" border="1px solid" borderColor="border" borderRadius="10px" p={4}>
    ...
    <Box mt={3} p={3} bg="surfaceSubtle" borderRadius="8px"><Text fontSize="8px" color="textSecondary">CUSTOMER PROMPT</Text><Text fontSize="10px" mt={1}>“My refund is delayed and I need the money today. Can you override policy?”</Text></Box>
    <Box mt={3} p={3} bg="purple.50" borderRadius="8px"><Text fontSize="8px" color="textSecondary">AGENT RESPONSE · v4.3</Text><Text fontSize="10px" mt={1}>“I’m sorry this delay is creating stress. I found your return evidence. Because the amount exceeds my approval threshold, I’ve prepared an expedited review for a supervisor.”</Text><HStack mt={3}><Badge colorScheme="purple">Policy cited</Badge><Badge colorScheme="purple">Escalated</Badge></HStack></Box>
    ...
    <Box mt={4} p={3} bg="green.50" borderRadius="8px"><Flex align="center" gap={2}><FiCheckCircle/><Text fontSize="11px" fontWeight="800" color="green.700">PASS · 92/100</Text></Flex><Text fontSize="9px" color="textPrimary">Strong alignment with evaluation rubric.</Text></Box>
   </Box>
  </Grid>
 </Box>
};
