...
const ConversationView=()=>{
 ...
 return <><Flex h="calc(100vh - 0px)" minH="700px" bg="surfaceSubtle" overflow="hidden">
  <Box w="58px" bg="surface" borderRight="1px solid" borderColor="border" display={{base:'none',md:'flex'}} flexDir="column" alignItems="center" py={5} gap={3}>
   {[
   ...
   <Tooltip label="Settings" placement="right">
    <Box as="button" type="button" aria-label="Settings" w="34px" h="34px" borderRadius="7px" display="flex" alignItems="center" justifyContent="center" color="textPrimary" _hover={{bg:'gray.50',color:'brand.600'}} onClick={()=>nav('/settings')}>
     <Icon as={FiSettings} boxSize={4}/>
    </Box>
   </Tooltip>
  </Box>
  <Box w={{base:'230px',lg:'245px'}} bg="surface" borderRight="1px solid" borderColor="border" p={3} display={{base:'none',md:'block'}}>
   <Text fontSize="10px" fontWeight="800" color="textSecondary" mb={2}>CUSTOMER CONVERSATIONS</Text>
   <HStack border="1px solid" borderColor="border" borderRadius="6px" px={2} mb={3}><Icon as={FiSearch} color="textSecondary"/><Input variant="unstyled" fontSize="10px" placeholder="Search customer or case..."/></HStack>
   <VStack spacing={2} align="stretch">{list.slice(0,6).map((c,i)=><Box key={c.id||c._id||i} p={2.5} border="1px solid" borderColor={String(c.id||c._id)===String(id)?'brand.300':'gray.200'} borderRadius="8px" cursor="pointer" bg={String(c.id||c._id)===String(id)?'brand.50':'white'} onClick={()=>select(c)}><Flex gap={2} align="center"><Avatar size="xs" name={c.customer?.name}/><Box flex="1" minW={0}><Text fontSize="10px" fontWeight="800" noOfLines={1}>{c.customer?.name||'Customer'}</Text><Text fontSize="8px" color="textSecondary" noOfLines={1}>{c.tags?.[0]||'Support request'}</Text></Box><Text fontSize="8px" color="textSecondary">{i?'09:18':'12:42'}</Text></Flex><Badge mt={2} fontSize="8px" colorScheme={c.alertLevel==='high'?'red':c.alertLevel==='medium'?'yellow':'purple'}>{c.alertLevel==='high'?'Critical':c.alertLevel==='medium'?'High':'Watch'}</Badge></Box>)}</VStack>
  </Box>
  <Box flex="1" minW={0} p={{base:3,md:4}} overflowY="auto">
   <Flex bg="surface" border="1px solid" borderColor="border" borderRadius="10px" minH="calc(100vh - 32px)" overflow="hidden" direction={{base:'column',xl:'row'}}>
    <Box flex="1.55" minW={0} borderRight={{xl:'1px solid'}} borderColor="border">
     <Flex p={4} justify="space-between" align="center" borderBottom="1px solid" borderColor="border"><HStack><Avatar size="sm" name={conv.customer?.name}/><Box><Text fontWeight="800" fontSize="12px">{conv.customer?.name} · Case #{String(conv.id||conv._id||'84291').slice(-5)}</Text><Text fontSize="8px" color="textSecondary">Returns · Web chat · EN-US · VIP tier</Text></Box></HStack><HStack><Badge colorScheme="red" fontSize="8px">● {conv.alertLevel==='high'?'Critical':'High'}</Badge>{taken?<Button size="xs" colorScheme="orange" onClick={release}>Return AI</Button>:<Button size="xs" onClick={take}>Take over</Button>}</HStack></Flex>
     <Box px={4} py={2.5} bg="yellow.50" borderBottom="1px solid" borderColor="yellow.100"><Text fontSize="9px" fontWeight="800">⚠ SLA breach likely in 03:18</Text><Text fontSize="8px" color="textPrimary">AI confidence fell after policy exception request</Text></Box>
     <VStack p={4} spacing={3} align="stretch">{messages.map((m,i)=><Flex key={i} justify={m.sender==='customer'?'start':'end'}><Box maxW="78%" bg={m.sender==='customer'?'gray.100':m.sender==='supervisor'?'brand.50':'purple.50'} px={3} py={2.5} borderRadius="9px"><Text fontSize="7px" color="textSecondary" fontWeight="800" mb={1}>{m.sender==='customer'?'CUSTOMER':m.sender==='supervisor'?'SUPERVISOR':'AI AGENT'} · {i+1}{m.sender!=='customer'&&' · 11:32'}</Text><Text fontSize="10px" lineHeight="1.5">{m.text}</Text></Box></Flex>)}</VStack>
     <Box mx={4} mb={3} p={3} bg="surfaceSubtle" border="1px solid" borderColor="border" borderRadius="9px"><Flex justify="space-between" align="center"><Box><Text fontSize="9px" fontWeight="800">Co-pilot recommendation</Text><Text fontSize="8px" color="textSecondary">Generated from policy, customer tier, and conversation sentiment</Text></Box><Badge colorScheme="green" fontSize="8px">● Confidence 88%</Badge></Flex><Text fontSize="8px" mt={2}>Approve expedited refund of ₹8,420 and waive the standard review period.</Text><Text fontSize="7px" color="textSecondary" mt={1}>Evidence: return scan received · item category eligible · customer lifetime value: high</Text><HStack mt={2}><Button size="xs" onClick={()=>setText('I’ve reviewed the return evidence and approved an expedited refund.')}>Approve action</Button><Button size="xs" variant="outline" onClick={()=>setText('I’m reviewing the evidence and will update you shortly.')}>Edit response</Button><Button size="xs" variant="outline">Escalate policy</Button></HStack></Box>
     <Box mx={4} mb={4} p={3} border="1px solid" borderColor="border" borderRadius="9px"><Flex justify="space-between" mb={2}><Text fontSize="9px" fontWeight="800">Supervisor response</Text><Button size="xs" variant="ghost" rightIcon={<FiChevronDown/>} onClick={()=>setTemplateOpen(true)}>Template</Button></Flex><Textarea value={text} onChange={e=>setText(e.target.value)} placeholder={taken?'Type or dictate a supervisor response...':'Take over to send a supervisor response.'} isDisabled={!taken} rows={2} fontSize="9px"/><Flex justify="space-between" mt={2}><Button size="xs" variant={listening?"solid":"outline"} leftIcon={listening?<FiMicOff/>:<FiMic/>} isDisabled={!taken} onClick={startVoice}>{listening?"Listening…":"Voice input"}</Button><Button size="xs" leftIcon={<FiSend/>} isDisabled={!taken||!text.trim()} onClick={send}>Send reply</Button></Flex></Box>
    </Box>
    <Box w={{base:'100%',xl:'275px'}} p={4} bg="surface">
     <Heading size="xs">Customer intelligence</Heading><HStack mt={3}><Avatar size="sm" name={conv.customer?.name}/><Box><Text fontWeight="800" fontSize="10px">{conv.customer?.name} · Case #{String(conv.id||conv._id||'84291').slice(-5)}</Text><Text fontSize="8px" color="textSecondary">VIP · 3.8 years · Madrid</Text></Box></HStack><Divider my={3}/><Flex justify="space-between"><Box><Text fontSize="7px" color="textSecondary">LIFETIME VALUE</Text><Text fontWeight="800" fontSize="13px">₹1.24L</Text></Box><Box><Text fontSize="7px" color="textSecondary">ORDERS</Text><Text fontWeight="800" fontSize="13px">38</Text></Box></Flex><Flex justify="space-between" mt={3}><Box><Text fontSize="7px" color="textSecondary">RETURN RATE</Text><Text fontWeight="800" fontSize="11px">5.2%</Text></Box><Box><Text fontSize="7px" color="textSecondary">SENTIMENT</Text><Text fontWeight="800" fontSize="11px" color="red.500">−0.62</Text></Box></Flex><Divider my={4}/><Text fontSize="10px" fontWeight="800">Case diagnostics</Text><Text fontSize="7px" color="textSecondary">Signals contributing to risk</Text>{[['Repeat contact',92,'red'],['Negative sentiment',76,'orange'],['Policy exception',66,'blue'],['Churn propensity',58,'purple']].map(([n,v,c])=><Box mt={3} key={n}><Flex justify="space-between" fontSize="8px"><Text>{n}</Text><Text>{v}%</Text></Flex><Progress mt={1} value={v} size="xs" colorScheme={c}/></Box>)}<Divider my={4}/><Text fontSize="8px" color="textSecondary" mb={1}>Feedback Notes</Text><Textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Write here..." h="110px" fontSize="9px" bg="yellow.50" borderColor="yellow.100"/><Button mt={3} w="100%" size="sm" leftIcon={<FiCheckCircle/>} onClick={()=>toast({title:'Case marked resolved',status:'success'})}>Mark as Resolved</Button>
    </Box>
   </Flex>
  </Box>
 </Flex>
   ...
   <ModalContent borderRadius="14px" overflow="hidden">
    <ModalBody p={0}>
     <Grid templateColumns="155px 1fr 245px" minH="500px">
      <Box bg="surfaceSubtle" p={4} borderRight="1px solid" borderColor="border">
       <Text fontSize="9px" fontWeight="900" mb={3}>TEMPLATE LIBRARY</Text>
       <Input size="sm" bg="surface" value={templateSearch} onChange={e=>setTemplateSearch(e.target.value)} placeholder="Search categories"/>
       ...
       <Text fontSize="8px" fontWeight="900" color="textSecondary" mt={5}>BY JOURNEY</Text>
       {['Onboarding','Billing','Engagement','Transaction'].map(x=><Text key={x} fontSize="9px" py={1.5}>● {x}</Text>)}
       <Text fontSize="8px" fontWeight="900" color="textSecondary" mt={4}>BY CHANNEL</Text>
       {['Email','Website','Mobile','Messenger'].map(x=><Text key={x} fontSize="9px" py={1.5}>● {x}</Text>)}
      </Box>
      <Box p={4}>
       <Flex justify="space-between" align="center" mb={2}><Box><Heading size="sm">Response Templates</Heading><Text fontSize="9px" color="textSecondary">Choose a reply and customize it before inserting.</Text></Box><Badge>{visibleTemplates.length} results</Badge></Flex>
       ...
       <Grid templateColumns="1fr 1fr" gap={3}>{visibleTemplates.map(t=><Box key={t.id} p={3} border="1px solid" borderColor={selectedTemplate.id===t.id?'brand.400':'gray.200'} borderRadius="9px" cursor="pointer" onClick={()=>setSelectedTemplate(t)} _hover={{borderColor:'brand.300'}}><Flex justify="space-between"><Box h="7px" w="70%" bg="gray.100" borderRadius="full"/>{t.favorite?<FiStar/>:<Text color="gray.300">☆</Text>}</Flex><Text fontSize="10px" fontWeight="800" mt={3}>{t.name}</Text><Text fontSize="8px" color="textSecondary" noOfLines={2}>{t.content}</Text><HStack mt={2}><Badge fontSize="7px">{t.category}</Badge><Badge colorScheme="purple" fontSize="7px">{t.channel}</Badge></HStack></Box>)}</Grid>
      </Box>
      <Box p={4} borderLeft="1px solid" borderColor="border">
       <Text fontSize="9px" fontWeight="900">Preview</Text><Text fontSize="8px" color="textSecondary">Review the selected reply before inserting.</Text>
       ...
       <Box mt={3} p={3} bg="surfaceSubtle" borderRadius="9px"><Badge mb={2} colorScheme="purple">Live preview</Badge><Text fontSize="9px" lineHeight="1.6">{resolvedTemplate}</Text><Button mt={3} size="xs" w="100%">View getting started</Button></Box>
       <Box mt={4} p={3} bg="green.50" borderRadius="8px"><Text fontSize="9px" fontWeight="800" color="green.700">✓ {selectedTemplate?.vars?.length||0} variables resolved</Text><Text fontSize="8px" color="textPrimary">You can edit the message after inserting.</Text></Box>
