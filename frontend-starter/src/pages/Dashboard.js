...
const Card=({children,...p})=><Box bg="surface" border="1px solid" borderColor="border" borderRadius="10px" {...p}>{children}</Box>;
const Metric=({label,value,change,icon,color,down})=><Card p={4} borderTop="3px solid" borderTopColor={color}><Flex justify="space-between"><Box><Text fontSize="10px" color="textSecondary" fontWeight="800" textTransform="uppercase" letterSpacing=".35px">{label}</Text><Heading mt={2} size="lg">{value}</Heading><HStack mt={1}><Icon as={FiArrowUpRight} transform={down?'rotate(90deg)':'none'} color={color}/><Text fontSize="10px" color={color}>{change}</Text><Text fontSize="10px" color="textSecondary">vs last period</Text></HStack></Box><Box p={2.5} bg="surfaceSubtle" borderRadius="8px" color={color}><Icon as={icon} boxSize={5}/></Box></Flex></Card>;
...
const Dashboard=()=>{
 ...
 return <Box>
  <Flex justify="space-between" align={{base:'start',md:'center'}} mb={5} direction={{base:'column',md:'row'}} gap={3}>
   <Box><Text fontSize="10px" color="textSecondary" fontWeight="800" letterSpacing=".5px">SUPERVISOR CONTROL ROOM</Text><Heading size="lg" mt={1}>Dashboard</Heading><Text color="textSecondary" fontSize="13px">Monitor conversations, agent health, and escalation risk.</Text></Box>
